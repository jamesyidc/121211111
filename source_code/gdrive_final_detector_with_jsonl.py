...
def parse_txt_content(content, snapshot_time):
    ...
    try:
        ...
        log(f"✅ 解析成功: {len(coin_snapshots)} 个币种, 急涨={rush_up_total}, 急跌={rush_down_total}")
...
def save_to_jsonl(coin_snapshots, aggregate_data):
    ...
    try:
        ...
        # 保存聚合数据
        agg_manager = AggregateJSONLManager(JSONL_DATA_DIR)
        agg_manager.append_aggregate(aggregate_data)
