...
def save_to_database(data: Dict) -> bool:
    ...
    try:
        ...
        level_data = {
            ...
            'support_line_2': data['support_line_2'],
            'resistance_line_1': data['resistance_line_1'],
            'resistance_line_2': data['resistance_line_2'],
            'distance_to_support_1': data['distance_to_support_1'],
            'distance_to_support_2': data['distance_to_support_2'],
            'distance_to_resistance_1': data['distance_to_resistance_1'],
            'distance_to_resistance_2': data['distance_to_resistance_2'],
            'position_s2_r1': data['position_s2_r1'],
            'position_s1_r2': data['position_s1_r2'],
            'position_s1_r2_upper': data['position_s1_r2_upper'],
            'position_s1_r1': data['position_s1_r1'],
            'position_7d': data['position_7d'],
            'position_48h': data['position_48h'],
            'support_1_days': 7,  # 7天周期
            'support_2_hours': 48,  # 48小时周期
            'resistance_1_days': 7,
            'resistance_2_hours': 48,
            'alert_scenario_1': int(data['alert_scenario_1']),
            'alert_scenario_2': int(data['alert_scenario_2']),
            'alert_scenario_3': int(data['alert_scenario_3']),
            'alert_scenario_4': int(data['alert_scenario_4']),
            'alert_7d_low': int(data['alert_7d_low']),
            'alert_7d_high': int(data['alert_7d_high']),
            'alert_48h_low': int(data['alert_48h_low']),
            'alert_48h_high': int(data['alert_48h_high']),
            'alert_triggered': int(data['alert_triggered']),
            'baseline_price_24h': data['baseline_price_24h'],
    ...
    while True:
        try:
            ...
            if current_minute in [0, 30] and current_second < 30:
                ...
                log(f"⏳ 下次采集时间: {next_time.strftime('%Y-%m-%d %H:%M')}, 等待 {wait_minutes} 分钟 {60-current_second} 秒...")
                time.sleep(min(60, wait_seconds))  # 每次最多等待60秒，方便中断
