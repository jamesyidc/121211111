...
async function loadAccounts() {
  try {
    ...
    const response = await fetch('/api/live-trading-v2/accounts/list');
...
function updateAccountDisplay() {
  ...
  if (account) {
    accountName.textContent = account.name;  // ✅ 从 account_name 改为 name
...
async function updatePrice() {
  ...
  try {
    const response = await fetch(`/api/live-trading-v2/ticker/${state.currentSymbol}?accountId=${state.currentAccountId}`);
    const data = await response.json();
...
function setLeverage(leverage) {
  ...
  // 更新限价面板按钮样式
  [3, 5, 10, 20, 30, 40, 50].forEach(lev => {
  ...
  // 更新市价面板按钮样式
  [3, 5, 10, 20, 30, 40, 50].forEach(lev => {
    const btn = document.getElementById(`market-lev${lev}`);
    if (btn) {
      if (lev === leverage) {
        btn.className = 'leverage-btn active';
      } else {
...
async function loadAccountInfo() {
  ...
  try {
    ...
    const response = await fetch(`/api/live-trading-v2/account?accountId=${state.currentAccountId}`);
    const data = await response.json();
...
async function loadPositions() {
  ...
  try {
    const response = await fetch(`/api/live-trading-v2/positions?accountId=${state.currentAccountId}`);
    const data = await response.json();
      ...
      container.innerHTML = data.positions.map(pos => {
        ...
        <div class="border border-gray-200 rounded-lg p-3">
          <div class="flex justify-between items-start mb-2">
            <div>
              ...
              <div class="text-xs ${pos.side === 'long' ? 'text-green-600' : 'text-red-600'}">
                ...
                ${hasTPSL ? '<span class="ml-1 text-blue-600" title="已设置止盈止损"><i class="fas fa-shield-alt"></i></span>' : ''}
              </div>
            </div>
            <div class="text-right">
          ...
          <div class="text-xs text-gray-600 space-y-1">
            ...
            <div>数量: ${pos.size} 张 (${(pos.usdtAmount || 0).toFixed(2)} USDT)</div>
          </div>
          <div class="mt-2 space-y-1">
            <div class="flex gap-1">
              ...
              <button 
                ...
                onclick="checkTPSLStatus('${pos.id}')"
                title="查看止盈止损状态"
              >
              ...
              <button 
                ...
                onclick="openTPSLModal('${pos.id}', '${pos.symbol}', '${pos.side}')"
                title="${hasTPSL ? '编辑止盈止损' : '设置止盈止损'}"
              >
                <i class="fas fa-chart-line"></i> ${hasTPSL ? '编辑' : '止盈止损'}
              </button>
              <button 
                class="flex-1 py-1 text-xs bg-red-500 text-white rounded hover:bg-red-600"
            ...
            <div class="flex gap-1">
              <button 
                class="flex-1 py-1 text-xs bg-orange-500 text-white rounded hover:bg-orange-600"
                onclick="reversePosition('${pos.id}', '${pos.symbol}', '${pos.side}', ${pos.leverage}, ${pos.usdtAmount || 0})"
                title="平掉当前仓位并开相反方向"
              >
                <i class="fas fa-exchange-alt"></i> 反手
              </button>
              <button 
                class="flex-1 py-1 text-xs bg-purple-500 text-white rounded hover:bg-purple-600"
                onclick="openDefenseAddModal('${pos.id}', '${pos.symbol}', '${pos.side}', ${pos.entryPrice}, ${pos.leverage})"
                title="设置防守加仓"
              >
                <i class="fas fa-shield-alt"></i> 防守加仓
              </button>
              <button 
                class="flex-1 py-1 text-xs bg-indigo-500 text-white rounded hover:bg-indigo-600"
                onclick="viewDefenseOrders('${pos.id}', '${pos.symbol}')"
                title="查看防守单列表"
                id="viewDefense_${pos.id}"
              >
                <i class="fas fa-list"></i> 防守单
              </button>
            </div>
          </div>
        </div>
...
async function submitOrder() {
  ...
  const endpoint = state.orderType === 'limit' ? 
    '/api/live-trading-v2/order/limit' : 
    '/api/live-trading-v2/order/market';
...
async function closePosition(positionId) {
  // 显示百分比选择对话框
  const percentage = await showClosePercentageDialog();
  if (!percentage) return; // 用户取消
  
  try {
    const response = await fetch(`/api/live-trading-v2/position/${positionId}/close?accountId=${state.currentAccountId}&percentage=${percentage}`, {
      method: 'POST'
    });
    ...
    if (data.success) {
      const msg = percentage === 100 ? '全部平仓成功！' : `平仓 ${percentage}% 成功！`;
...
// 显示平仓百分比选择对话框
function showClosePercentageDialog() {
  return new Promise((resolve) => {
    ...
    dialog.innerHTML = `
      <div style="background: white; padding: 30px; border-radius: 12px; max-width: 400px; width: 90%;">
        <h3 style="margin: 0 0 20px 0; font-size: 20px; color: #333;">选择平仓比例</h3>
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px;">
          <button class="percentage-btn" data-percentage="30" style="padding: 16px; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; font-size: 16px; font-weight: 600; transition: all 0.2s;">
            30%
          </button>
          <button class="percentage-btn" data-percentage="50" style="padding: 16px; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; font-size: 16px; font-weight: 600; transition: all 0.2s;">
            50%
          </button>
          <button class="percentage-btn" data-percentage="100" style="padding: 16px; border: 2px solid #3b82f6; border-radius: 8px; background: #eff6ff; cursor: pointer; font-size: 16px; font-weight: 600; transition: all 0.2s; color: #3b82f6;">
            100%
          </button>
        </div>
        <div style="display: flex; gap: 12px;">
          <button id="confirmCloseBtn" style="flex: 1; padding: 12px; background: #10b981; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 600;">
            确认平仓
          </button>
          <button id="cancelCloseBtn" style="flex: 1; padding: 12px; background: #ef4444; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 600;">
            取消
          </button>
        </div>
      </div>
    ...
    let selectedPercentage = 100; // 默认100%
    
    // 百分比按钮点击事件
    const percentageBtns = dialog.querySelectorAll('.percentage-btn');
    percentageBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        percentageBtns.forEach(b => {
        ...
        selectedPercentage = parseInt(btn.dataset.percentage);
      });
    });
    
    // 确认按钮
    dialog.querySelector('#confirmCloseBtn').addEventListener('click', () => {
      document.body.removeChild(dialog);
      resolve(selectedPercentage);
    });
    
    // 取消按钮
    dialog.querySelector('#cancelCloseBtn').addEventListener('click', () => {
      document.body.removeChild(dialog);
      resolve(null);
    });
...
// 反手操作
async function reversePosition(positionId, symbol, currentSide, leverage, usdtAmount) {
  // 显示确认对话框
  const confirmed = await showReverseConfirmDialog(symbol, currentSide, leverage, usdtAmount);
  if (!confirmed) return;
  
  try {
    showToast('正在执行反手操作...', 'info');
    
    // 步骤1：平掉当前持仓（100%）
    const closeResponse = await fetch(`/api/live-trading-v2/position/${positionId}/close?accountId=${state.currentAccountId}&percentage=100`, {
      method: 'POST'
    });
    ...
    if (!closeData.success) {
      showToast(`平仓失败: ${closeData.error}`, 'error');
      return;
    }
    ...
    // 步骤2：开相反方向的仓位
    const reverseSide = currentSide === 'long' ? 'sell' : 'buy';
    const orderData = {
      accountId: state.currentAccountId,
      symbol: symbol,
      side: reverseSide,
      ...
      orderType: 'market' // 反手使用市价单，确保快速成交
    };
    
    const openResponse = await fetch('/api/live-trading-v2/order', {
      method: 'POST',
      headers: {
    ...
    if (openData.success) {
      showToast(`反手成功！已平${currentSide === 'long' ? '多' : '空'}开${reverseSide === 'buy' ? '多' : '空'} ${symbol}`, 'success');
      loadAccountInfo();
      loadPositions();
    } else {
      showToast(`开仓失败: ${openData.error}，但已平仓成功`, 'warning');
      loadAccountInfo();
      loadPositions();
    }
  } catch (error) {
    showToast(`反手失败: ${error.message}`, 'error');
  }
}

// 显示反手确认对话框
function showReverseConfirmDialog(symbol, currentSide, leverage, usdtAmount) {
  return new Promise((resolve) => {
    const dialog = document.createElement('div');
    dialog.style.cssText = `
      position: fixed;
    ...
    const sideText = currentSide === 'long' ? '做多' : '做空';
    const reverseSideText = currentSide === 'long' ? '做空' : '做多';
    const sideColor = currentSide === 'long' ? '#10b981' : '#ef4444';
    const reverseSideColor = currentSide === 'long' ? '#ef4444' : '#10b981';
    
    dialog.innerHTML = `
      <div style="background: white; padding: 30px; border-radius: 12px; max-width: 450px; width: 90%;">
        <h3 style="margin: 0 0 20px 0; font-size: 22px; color: #333; text-align: center;">
          <i class="fas fa-exchange-alt" style="color: #f59e0b;"></i> 确认反手操作
        </h3>
        
        <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
            <div style="text-align: center; flex: 1;">
              <div style="font-size: 14px; color: #6b7280; margin-bottom: 5px;">当前持仓</div>
              <div style="font-size: 18px; font-weight: bold; color: ${sideColor};">
                ${sideText} ${symbol}
              </div>
              <div style="font-size: 12px; color: #6b7280; margin-top: 3px;">
                ${leverage}x 杠杆 | ${usdtAmount.toFixed(2)} USDT
              </div>
            </div>
            
            <div style="font-size: 28px; color: #f59e0b; margin: 0 10px;">
              <i class="fas fa-arrow-right"></i>
            </div>
            
            <div style="text-align: center; flex: 1;">
              <div style="font-size: 14px; color: #6b7280; margin-bottom: 5px;">反手后</div>
              <div style="font-size: 18px; font-weight: bold; color: ${reverseSideColor};">
                ${reverseSideText} ${symbol}
              </div>
              <div style="font-size: 12px; color: #6b7280; margin-top: 3px;">
                ${leverage}x 杠杆 | ${usdtAmount.toFixed(2)} USDT
              </div>
        ...
        <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px; border-radius: 4px; margin-bottom: 20px;">
          <div style="color: #92400e; font-size: 13px; line-height: 1.5;">
            <strong><i class="fas fa-exclamation-triangle"></i> 操作说明：</strong><br>
            1. 先平掉当前持仓（100%平仓）<br>
            2. 然后立即开相反方向的仓位<br>
            3. 使用相同的杠杆和金额
          </div>
        ...
        <div style="display: flex; gap: 12px;">
          <button id="confirmReverseBtn" style="flex: 1; padding: 14px; background: #f59e0b; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 600;">
            <i class="fas fa-check"></i> 确认反手
          </button>
          <button id="cancelReverseBtn" style="flex: 1; padding: 14px; background: #6b7280; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 600;">
    ...
    // 确认按钮
    dialog.querySelector('#confirmReverseBtn').addEventListener('click', () => {
      document.body.removeChild(dialog);
      resolve(true);
    });
    
    // 取消按钮
    dialog.querySelector('#cancelReverseBtn').addEventListener('click', () => {
      document.body.removeChild(dialog);
      resolve(false);
    });
...
async function saveAccount() {
  ...
  try {
    const response = await fetch('/api/live-trading-v2/accounts/add', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function testAPIConnection() {
  ...
  try {
    const response = await fetch('/api/live-trading-v2/test-connection', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function testAccount(accountId) {
  ...
  try {
    const response = await fetch(`/api/live-trading-v2/accounts/${accountId}/test`);
    const data = await response.json();
...
async function deleteAccount(accountId) {
  ...
  try {
    const response = await fetch(`/api/live-trading-v2/accounts/${accountId}`, {
      method: 'DELETE'
    });
...
async function submitTPSL(event) {
  ...
  try {
    const response = await fetch('/api/live-trading-v2/position/set-tpsl', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function submitDefenseAdd(event) {
  ...
  try {
    const response = await fetch('/api/live-trading-v2/position/defense-add', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        accountId: state.currentAccountId,
